...
def calculate_pressure(input_dir, output_dir, replica):
    if not os.path.exists("%spressure_%s.xvg" % (output_dir, replica)):
        order = "echo 'Pressure' | %s energy -f %s*.edr -o %spressure_%s.xvg" % (gmx, input_dir, output_dir, replica)
        subprocess.call(order, shell=True)


def read_pressure(output_dir, replica):
    file_dir = "%spressure_%s.xvg" % (output_dir, replica)
    data = []
    time_series = []
    values = []
    file = open(file_dir)
