...
def calculate_h_bonds_local_inter(input_dir, output_dir, replica, residue):
    if not os.path.isfile("%sH_bonds_resid%s_inter_%s.xvg" % (output_dir, str(residue), replica)):
...
def read_h_bonds_local_inter(output_dir, replica, residue):
    file_dir = "%sH_bonds_resid%s_inter_%s.xvg" % (output_dir, str(residue), replica)
    data = []
    time_series = []
    values = []
    file = open(file_dir)
