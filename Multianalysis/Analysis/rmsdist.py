...
def calculate_rmsdist(input_dir, output_dir, replica):
    if not os.path.exists("%srmsdist_%s.xvg" % (output_dir, replica)):
        order = "echo 'Protein' | %s rmsdist -f %s*.xtc -s %s*.gro -o %srmsdist_%s.xvg" \
                % (gmx, input_dir, input_dir, output_dir, replica)
        subprocess.call(order, shell=True)


def read_rmsdist(output_dir, replica):
    file_dir = "%srmsdist_%s.xvg" % (output_dir, replica)
    data = []
    time_series = []
    values = []
    file = open(file_dir)
